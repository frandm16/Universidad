...
{ // multiplica una parte de la matriz según el identificador de thread
    int n, i, j, k, inicio, final;

    n = *(int *)p;
    inicio = (SIZE / THREADS) * n;
    if (n != THREADS - 1)
        final = (SIZE / THREADS) * (n + 1) - 1;
    else
        final = SIZE - 1; // el último se queda con el resto
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < THREADS; i++)
        thid[i] = i;
    for (i = 0; i < THREADS; i++)
        pthread_create(&tid[i], NULL, multiplica, (void *)&thid[i]);
    for (i = 0; i < THREADS; i++)
        pthread_join(tid[i], NULL); // esperamos a todos
    gettimeofday(&end, NULL);
    printf("|--- tiempo transcurrido: %0.2f ms\n", ((end.tv_sec * 1000.0 + end.tv_usec / 1000.0) - (start.tv_sec * 1000.0 + start.tv_usec / 1000.0)));
